...
{
    public class AuthApiService : IAuthenticationApiService
    {
        private readonly HttpClient _client;
        private readonly ILocalStorageService _localStorage;
        private readonly AuthenticationStateProvider _authStateProvider;

        public AuthApiService(HttpClient client, ILocalStorageService localStorage, AuthenticationStateProvider authStateProvider)
        {
            _client = client;
        ...
        {
            var response = await _client.GetAsync($"api/auth/getbyid/{userId}");
        ...
        {
            var response = await _client.GetAsync("api/auth/get");
            var contentTemp = await response.Content.ReadAsStringAsync();
        ...
        public async Task<LoginResponseDto> Login(LoginRequestDto userFromAuthentication)
        {
            ...
            var response = await _client.PostAsync("api/auth/login", bodyContent);
            var contentTemp = await response.Content.ReadAsStringAsync();
        ...
        public async Task<ApiResponseDto> Logout()
        {
            ...
            var response = await _client.GetAsync($"api/auth/logout/{id}");
            if (!response.IsSuccessStatusCode)
            {
        ...
        public async Task<ApiResponseDto> RegisterUser(RegisterRequestDto userForRegisteration)
        {
            ...
            var bodyContent = new StringContent(content, Encoding.UTF8, "application/json");
            var response = await _client.PostAsync("api/auth/register", bodyContent);
            var contentTemp = await response.Content.ReadAsStringAsync();
        ...
        public async Task<ApiResponseDto> EditUserAsync(UserDetailsDto user)
        {
            ...
            var bodyContent = new StringContent(content, Encoding.UTF8, "application/json");
            var response = await _client.PostAsync("api/auth/edituser", bodyContent);
        ...
        public async Task<ApiResponseDto> UpsertUserAsync(UserDetailsDto user)
        {
            var content = JsonConvert.SerializeObject(user);
            var bodyContent = new StringContent(content, Encoding.UTF8, "application/json");
            var response = await _client.PostAsync("api/auth/upsertbyadmin", bodyContent);
        ...
        public async Task<ApiResponseDto> DeleteUserAsync(string userId)
        {
            ...
            var bodyContent = new StringContent(content, Encoding.UTF8, "application/json");
            var response = await _client.PostAsync("api/auth/delete", bodyContent);
            var contentTemp = await response.Content.ReadAsStringAsync();
            var result = JsonConvert.DeserializeObject<ApiResponseDto>(contentTemp);
