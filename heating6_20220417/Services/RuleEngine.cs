...
namespace Services
{
    public class RuleEngine : BackgroundService
    {
        ...
        private async void OnSaveMeasurements(object state)
        {
            ...
            {
                Log.Error($"RuleEngine,OnSaveMeasurements;failed to save measurements; ex: {ex.Message}");
            }
        }
        ...
        private async Task SaveMeasurements(IEnumerable<Measurement> measurements)
        {
            if (measurements.Any())
            {
                ...
                Log.Information($"RuleEngine;SaveMeasurements;{measurements.Count()} Measurements to save");
                using ApplicationDbContext dbContext = new(ConnectionString);
                using IUnitOfWork unitOfWork = new UnitOfWork(dbContext);
                try
                {
                ...
                {
                    Log.Error($"RuleEngine,SaveMeasurements;Failed to save Measurement; ex: {ex.Message}");
                }
            }
        }
